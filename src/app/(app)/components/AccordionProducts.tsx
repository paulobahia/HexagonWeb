...
function AccordionProducts({ sideBar }: { sideBar: boolean }) {
    ...
    return (
        <>
            <div onClick={openAccordionProducts} className={`flex items-center  ${sideBar ? "justify-between" : 'justify-center'}   px-2 py-3 hover:bg-zinc-400/30 rounded-lg transition-colors cursor-pointer `}>
                <div className={`flex items-center gap-2 ${!sideBar && "justify-center"} rounded-lg transition-all group duration-300 ease-in cursor-pointer`}>
                    ...
                    {!sideBar && (
                        <span className={`absolute left-12 mb-8 rounded-t-lg rounded-br-lg font-medium scale-0 bg-gray-50 p-2 text-xs  text-zinc-800 group-hover:scale-100`}>Produtos</span>
                    )}
                </div>
                {sideBar && (
