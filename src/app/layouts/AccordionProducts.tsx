...
function AccordionProducts({ sideBar }: { sideBar: boolean }) {
    ...
    return (
        <>
            <div onClick={openAccordionProducts} className={`flex items-center  ${sideBar ? "justify-between" : 'justify-center'} group px-2 py-3 hover:bg-zinc-400/30 rounded-lg transition-colors cursor-pointer `}>
                <div className={`flex items-center gap-2 ${!sideBar && "justify-center"} rounded-lg transition-all duration-300 ease-in cursor-pointer`}>
            ...
            <section className={`flex flex-col ${sideBar ? "px-7 py-1" : "items-center"}  justify-between gap-y-2 rounded-lg overflow-hidden transition-[max-height] duration-500 ease-in delay-0 ${openAccordion ? "max-h-40" : "max-h-0"}`} >
                <div onClick={goToOverview} className="flex items-center gap-x-3 p-2 cursor-pointer rounded-lg hover:bg-zinc-400/30 group">
                    ...
                    {!sideBar && (
                        <span className={`fixed left-12 mb-8 rounded-t-lg rounded-br-lg font-medium scale-0 bg-gray-50 p-2 text-xs  text-zinc-800 group-hover:scale-100`}>Overview</span>
                    )}
                </div>
                <div onClick={goToCreate} className="flex items-center gap-x-3 p-2 cursor-pointer rounded-lg hover:bg-zinc-400/30 group">
                    ...
                    {!sideBar && (
                        <span className={`fixed left-12 mb-8 rounded-t-lg rounded-br-lg font-medium scale-0 bg-gray-50 p-2 text-xs  text-zinc-800 group-hover:scale-100`}>Criar</span>
                    )}
                </div>
                <div onClick={goToEdit} className="flex items-center gap-x-3 p-2 cursor-pointer rounded-lg hover:bg-zinc-400/30 group">
                    ...
                    {!sideBar && (
                        <span className={`fixed left-12 mb-8 rounded-t-lg rounded-br-lg font-medium scale-0 bg-gray-50 p-2 text-xs  text-zinc-800 group-hover:scale-100`}>Editar</span>
                    )}
                </div>
            </section>
        </>
